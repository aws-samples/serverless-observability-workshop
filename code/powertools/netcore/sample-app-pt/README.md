...
```bash
sample-app-pt$ sam local invoke HelloWorldFunction --event events/event.json
```

The AWS SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
sample-app-pt$ sam local start-api
sample-app-pt$ curl http://localhost:3000/
```
...
```bash
sample-app-pt$ sam logs -n HelloWorldFunction --stack-name Logging --tail
```
